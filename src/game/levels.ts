...
function createLevel3(): LevelDef {
  ...
  // Diggable column at cols 12-13, rows 14-16 (top 3 rows only)
  // Two lowest tiles (rows 17-18) removed so NPCs fall cleanly to row 19 platform
  for (let r = 14; r <= 16; r++) {
  ...
  for (let c = 26; c <= 29; c++) setTile(map, ROWS - 1, c, 2); // architect gap
