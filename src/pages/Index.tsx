...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const drawCrucifiedMartyr = (
      ...
      if (tier >= 2) {
        const glowR = h * (tier === 3 ? 2.35 : 1.37);
        const glowAlpha = tier === 3 ? 0.43 : 0.21;
        ...
        if (visibleCount > 0) {
          ...
          for (let ti = 1 as 1 | 2 | 3; ti <= 3; ti++) {
            for (let mi = 0; mi < visibleCount; mi++) {
              ...
              if (m.tier === 1) {
                // On mega-distant ridge — single small figure — NO parallax drift
                const bell = Math.exp(-Math.pow((xr - 0.45) / 0.25, 2));
                const tilt = (xr - 0.5) * 4;
                ...
                // On distant hill — mid-sized — NO parallax drift
                ...
                // Foreground — large, advancing toward player — NO parallax drift
